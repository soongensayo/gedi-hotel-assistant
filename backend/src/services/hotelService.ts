...
function normalizeRoom(row: any) {
  ...
  return {
    ...
    roomForRobot: row.room_for_robot ?? row.roomForRobot ?? null,
  };
}
...
export async function getRoomWaypointId(roomId: string): Promise<string | null> {
  if (supabase) {
    const { data, error } = await supabase
      ...
      .eq('id', roomId)
      .single();
    if (!error && data) return data.room_for_robot ?? null;
  }
