...
/** Points ~5 o’clock (down-right) toward the physical passport slot. */
function PassportSlotArrow({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 44 58" fill="none" aria-hidden>
      <defs>
        <marker
          id="passport-slot-arrowhead"
          viewBox="0 0 10 10"
          refX="8"
          refY="5"
          markerWidth="6"
      ...
      <path
        ...
        strokeWidth="2"
        strokeLinecap="round"
        markerEnd="url(#passport-slot-arrowhead)"
      />
    </svg>
  );
}
...
function ScanningIndicator({ attempts }: { attempts: number }) {
  return (
    <>
      <div className="relative w-80 h-52 rounded-2xl border-2 border-dashed border-hotel-accent/30 bg-hotel-accent/5 flex items-center justify-center">
        <style>{`
          @keyframes scanLine {
            0%, 100% { top: 0; }
            50% { top: calc(100% - 4px); }
        ...
        <div
          className="pointer-events-none absolute z-20 right-2 bottom-2 flex flex-col items-end gap-1.5 pl-10 pt-6"
          aria-hidden
        >
          <span className="text-[0.65rem] font-medium uppercase tracking-[0.12em] text-hotel-accent/75">
            Reader
          </span>
          <PassportSlotArrow className="w-[3.25rem] h-[4.25rem] text-hotel-accent drop-shadow-sm" />
        </div>
      </div>
      <p className="text-hotel-text-dim text-sm text-center max-w-sm">
        Place your passport face-down in the slot below and to the right — it will be read automatically.
      </p>
      {attempts > 0 && (
